...
class StepWindow(QMainWindow):
    ...
    def __init__(self, session: Session, index: int, base_resultats: dict,
        ...
        self.setWindowTitle("Protocole de test — mode pas-à-pas")
    ...
    def _build(self):
        ...
        root.addWidget(QLabel("<b>Annotation / observations</b> (horodatee dans le rapport) :"))
        self.notes = QTextEdit()
        self.notes.setMaximumHeight(84)
        self.notes.setPlaceholderText(
            "Ce qui a ete observe, conditions particulieres, pistes... "
            "(obligatoire en pratique pour PAS BON / BLOQUE)")
    ...
    def _verdict(self, statut: str):
        ...
        if statut in (PAS_BON, BLOQUE) and not annotation:
            ...
            self.lbl_attendu.setText(
                "<b>Une annotation est fortement conseillee pour un echec</b> "
                "(qu'a-t-on observe ? dans quelles conditions ?) — puis re-clique le verdict.")
            return
        self.lbl_attendu.setStyleSheet(
            "background:#e8f5e9; border:1px solid #1a7f37; border-radius:6px; padding:8px;")
...
class BilanDialog(QDialog):
    def __init__(self, session: Session, parent=None):
        super().__init__(parent)
        self.session = session
        self.setWindowTitle("Protocole de test — bilan de session")
        ...
        root.addWidget(QLabel("<b>Echecs a traiter</b> (PAS BON + BLOQUES) :"))
        ...
        btns = QHBoxLayout()
        b_md = QPushButton("Exporter le bilan (Markdown)")
        ...
        b_gh = QPushButton("Ouvrir un ticket GitHub pour les echecs")
    ...
    def _export_markdown(self):
        ...
        QMessageBox.information(self, "Export", f"Bilan Markdown ecrit :\n{path}")
    ...
    def _export_csv(self):
        ...
        QMessageBox.information(self, "Export", f"Bilan CSV ecrit :\n{path}")
...
class MainWindow(QMainWindow):
    ...
    def _build(self):
        ...
        head = QHBoxLayout()
        self.lbl_session = QLabel("<b>Aucune session</b>")
    ...
    def _nouvelle_session(self):
        dialog = QDialog(self)
        dialog.setWindowTitle("Nouvelle session de test")
        dialog.setMinimumWidth(560)
        lay = QVBoxLayout(dialog)
        lay.addWidget(QLabel(
            "<b>Sur quelle base partir ?</b><br>"
            "En reprise, un cas modifie depuis (revision superieure) ou marque "
            "\"a retester\" est automatiquement remis a non testé — un test vert "
            "pris sur l'ancien code ne masque jamais une regression."))
        radios = {}
        r_vierge = QRadioButton("Session VIERGE (tout retester)")
        radios["vierge"] = r_vierge
        r_reprise = QRadioButton("Reprendre les statuts d'une session anterieure "
                                 "(cas modifies remis a zero) — recommande")
        radios["reprise"] = r_reprise
        r_total = QRadioButton("Reprendre une session en reinitialisant TOUT")
        ...
        combo = None
        if sessions:
            lay.addWidget(QLabel("Session de base :"))
        ...
        if radios["reprise"].isChecked() or radios["total"].isChecked():
            if combo is None:
                QMessageBox.information(self, "Nouvelle session",
                                        "Aucune session anterieure disponible.")
                return
