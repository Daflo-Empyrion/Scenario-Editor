...
- AUCUN Template source : valeurs LES PLUS COURANTES du fichier Templates.ecf
  (scan_template_defaults -- CraftTime/Target les plus frequents, ingredients
  les plus utilises avec leur quantite la plus commune).
...
d'ingredients (boutons dedies, gui/template_recipe_editor.py).
"""
...
def create_templates(parent, main_window, templates_path: Path,
    ...
    - sinon (nom absent, vide, ou Template introuvable) : chaque Template
      ...
      -- demande explicite de l'utilisateur du 30/08/2026 : pouvoir creer une
      recette meme quand le bloc duplique/cree n'en avait pas d'origine.

    Un apercu editable individuel (ajustement scalaires + ajout/suppression
    d'ingredients) est propose avant l'ecriture ; l'annuler signifie
    ...
    adjust_dialog = TemplateAdjustDialog(
        ...
        list(craftable_names or []), parent=parent)
    entries: dict = {}
    ...
    for variant_name in variant_names:
        ...
        if source_template is not None:
            ...
            new_template = create_new_block(defaults["kind"], None, variant_name,
                                             list(scalar_fields))
            if ingredients:
