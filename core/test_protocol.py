...
CASES = [
    ...
    # ---------------------------------------------------------------- TECH
    {
        "id": "TECH-001", "cat": "TECH", "rev": 3,
        "titre": "Arbre technologique : categories, tuiles vertes, mise en page adaptative",
        "etapes": ["Outils > Arbre technologique... (ou Ctrl+T ou la barre d'outils).", "Compare avec l'ecran F3 du jeu.",
                   "Observe les tuiles sous les icones (vertes, effet brillant)."],
        "attendu": "Categories dans l'ordre du jeu (Base, CV, SV, HV, Misc, Tools, Weapons), Hidden jamais affiche. Chaque icone pose sur une tuile verte style 'bouton' (cadre en degrade, reflet brillant).",
    },
    {
    ...
    {
        "id": "TECH-005", "cat": "TECH", "rev": 2,
        "titre": "Modification du niveau requis (UnlockLevel)",
        "etapes": ["Deplace une icone HORIZONTALEMENT vers une autre colonne de niveau.", "Verifie le fichier."],
        "attendu": "UnlockLevel ecrit dans le bon fichier (colonne reelle la plus proche) ; l'arbre le montre en direct.",
    },
    {
        "id": "TECH-006", "cat": "TECH", "rev": 2,
        "titre": "Modification du cout (UnlockCost)",
        "etapes": ["Clique une icone : la fiche s'ouvre ; clique la valeur du cout, change-la, valide.",
                   "Ou : clic droit sur l'icone > modifier le cout.", "Verifie le fichier."],
        "attendu": "UnlockCost mis a jour correctement ; le badge de l'icone et la fiche montrent la nouvelle valeur.",
    },
    {
    ...
    {
        "id": "FICHE-001", "cat": "TECH", "rev": 3,
        "titre": "Fiche info : ouverture au DOUBLE-clic sur un bloc (editeur ECF)",
        "etapes": ["Dans un onglet ECF de la copie de travail, clique UNE fois un bloc de l'arbre.",
                   "Puis double-clique le meme bloc."],
        "attendu": "Dans l'editeur ECF, un clic simple N'OUVRE PAS la fiche ; seul le double-clic affiche la fiche flottante (vue COMPLETE par defaut : toutes les proprietes, meme 'display: false').",
    },
    {
        "id": "FICHE-002", "cat": "TECH", "rev": 5,
        "titre": "Fiche info : vue COMPLETE par defaut + libelles et descriptions traduits",
        "etapes": ["Ouvre la fiche d'un bloc connu (ex: FuelTankMSLarge) : TOUTES les proprietes sont visibles, y compris celles en 'display: false'.",
    ...
    {
        "id": "TECH-014", "cat": "TECH", "rev": 2,
        "titre": "Arbre technologique : cout de deblocage visible sur chaque icone",
        "etapes": ["Outils > Arbre technologique...", "Observe le badge dore EN HAUT A GAUCHE de CHAQUE icone.", "Agrandis la fenetre : icone et badge grossissent ensemble.", "Survole une icone : l'infobulle doit confirmer le meme cout."],
        "attendu": "Chaque icone porte son UnlockCost (badge dore sur pastille sombre, EN HAUT A GAUCHE, police proportionnelle a l'icone, lisible sur tous les ecrans) ; le chiffre correspond a l'infobulle.",
    },
    {
        "id": "TECH-015", "cat": "TECH",
        "titre": "Arbre adaptatif : niveaux 1 a 25 visibles a toute resolution",
        "etapes": ["Outils > Arbre technologique...", "Reduisez la fenetre au minimum, puis agrandis-la en plein ecran (ou sur un second ecran d'une autre resolution)."],
        "attendu": "Les 9 jalons (Niveau 1 a Niveau 25) restent TOUJOURS tous visibles sans defilement horizontal ; icones, tuiles et badges s'agrandissent/reduisent avec la fenetre (bornes mini/maxi pour rester lisibles).",
    },
    {
        "id": "FICHE-005", "cat": "TECH", "rev": 2,
        "titre": "Fiche info : edition INLINE d'une valeur (editeur ECF)",
        "pre": "Onglet ECF de la copie de travail ouvert.",
        "etapes": ["Double-clique un bloc pour ouvrir sa fiche.", "Clique une valeur (ex: HitPoints) : une liste deroulante EDITABLE apparait (valeurs observees du fichier, tri frequence, fleche visible) avec la valeur BRUTE courante.",
                   "Choisis une valeur proposee OU tape-la librement, valide avec Entree."],
        "attendu": "Le fichier est modifie (marqueur ✎ *, compteur de modifications a jour, annotation d'origine), le tableau de proprietes ET la fiche montrent la nouvelle valeur ; annuler de l'onglet revient a l'ancienne valeur. Echap pendant la saisie annule sans ecrire. Sans historique pour la cle : saisie libre simple.",
    },
    {
        "id": "FICHE-006", "cat": "TECH", "rev": 2,
        "titre": "Fiche info : ajouter / supprimer une propriete",
        "etapes": ["Ouvre la fiche d'un bloc, clique '+ Ajouter une propriete' : les combos de cle et de valeur proposent les cles/valeurs observees du fichier (saisie libre possible), renseigne et valide.",
                   "Puis clic droit sur cette nouvelle ligne > Supprimer cette propriete."],
        "attendu": "La propriete est ajoutee au bloc (visible fiche + tableau + fichier, annulable) puis supprimee. Les proprietes de la ligne d'ouverture (Id, Name) ne sont jamais proposables a la suppression.",
    },
    {
        "id": "FICHE-007", "cat": "TECH", "rev": 2,
        "titre": "Fiche info : ingredients du Template editables (quantites, ajout, suppression)",
        "pre": "Un bloc dont le Template existe dans Templates.ecf.",
        "etapes": ["Ouvre la fiche du bloc, section FABRICATION.", "Clique une quantite d'ingredient, change-la, valide.",
                   "Ajoute un ingredient ('+ Ajouter un ingredient') : la combo propose les NOMS d'items/blocs (ItemsConfig + BlocksConfig, meme pool que la creation de Template) ; tape ou choisis-en un, saisis une quantite, valide.",
                   "Puis supprime-le (clic droit)."],
        "attendu": "Templates.ecf est mis a jour (onglet ouvert : il est marque modifie ; sinon ecriture disque directe avec annulation globale) ; la fiche rafraichit les quantites en direct.",
    },
    {
        "id": "FICHE-008", "cat": "TECH",
        "titre": "Fiche info editable depuis l'ARBRE technologique",
        "etapes": ["Outils > Arbre technologique...", "Clic simple (sans glisser) sur une icone : la fiche s'ouvre ; double-clic aussi.",
                   "Modifie une valeur (ex: le cout) et valide.", "Clique le bouton global Annuler."],
        "attendu": "La fiche s'ouvre sur le noeud, complete et editable. Chaque modification ecrit le fichier (annulation globale enregistree, onglet ouvert recharge, arbre et badge mis a jour en direct) ; Annuler restaure l'etat anterieur. Glisser une icone n'ouvre PAS la fiche.",
    },
    {
        "id": "FICHE-009", "cat": "TECH",
        "titre": "Fiche info : bascule 'Vue jeu (fidele F3)' / vue complete",
        "etapes": ["Ouvre la fiche d'un bloc vrai du jeu (avec attributs display).", "Coche 'Vue jeu (fidele F3)' en haut de la fiche, puis decoche-la.",
                   "Exporte la fiche en Markdown dans chaque mode."],
        "attendu": "Cochee : la fiche retombe sur la fide lite F3 du jeu (les 'display: false' sont masques). Decochee (defaut) : tout est visible. L'export Markdown reflete TOUJOURS la vue affichee.",
    },
    {
        "id": "ROBU-009", "cat": "ROBU", "rev": 2,
        ...
        "attendu": "Le pas-a-pas complet est disponible DANS l'application (meme moteur que tools/protocole_test.py) ; les verdicts sont sauvegardes entre deux lancements ; l'export ecrit un bilan complet. Barres 'Preparation' et 'Resultat attendu' LISIBLES sur themes clairs ET sombres (texte sombre sur fond clair).",
    },
]
